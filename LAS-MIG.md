...
För **Confirm sign up**, ladda först upp paketets nya `famili-confirmation.html` och `famili-confirmation.js`. Ersätt därefter mejlets Body i Supabase med innehållet i `docs/supabase-confirmation-template.html` från appprojektet. Mejllänken börjar då med jrdevelopment.se och innehåller `{{ .TokenHash }}`. På webbsidan trycker användaren på Bekräfta min e-post, och sidan verifierar kontot hos Supabase innan den visar att kontot är bekräftat. Länken får inte ersättas av en vanlig webbadress utan token_hash.

För **Reset password** används fortfarande `{{ .ConfirmationURL }}` med den befintliga återställningssidan.
